...
function towersPush(pile, disk) {
  TAJS_dumpValue(pile); // UInt|NaN
  TAJS_dumpValue(disk); // [@Object#node171, *Object#node171]
...
function towersPop(pile) {
  TAJS_dumpValue(pile); // Undef|UInt|NaN
  var top = towersPiles[pile];
//  if (top == null) error("Attempting to remove a disk from an empty pile");
  TAJS_dumpValue(top.next); // Undef|Null|[@Object#node171, *Object#node171]
...
function towersMoveTop(from, to) {
  TAJS_dumpValue(from); // Undef|UInt|NaN
  TAJS_dumpValue(to); // UInt|NaN
...
towersMovesDone = 0;
towersMove(0, 1, 13);
TAJS_dumpValue(towersMovesDone); // UInt
//  if (towersMovesDone != 8191) 
//    error("Error in result: " + towersMovesDone + " should be: 8191");
TAJS_dumpObject(towersPiles); // {0:Undef|Null|[@Object#node171, *Object#node171]|absent,
