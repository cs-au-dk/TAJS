...
public class TestJQueryUse_unanalyzable {
    ...
    private void test(String testCase) {
        ...
        // Successful tests runs in less than 1 minute on `Intel(R) Core(TM) i7-3520M CPU @ 2.90GHz`
        AnalysisTimeLimiter analysisLimiter = new AnalysisTimeLimiter(90);
        try {
            Misc.run("benchmarks/tajs/src/jquery/test-suite/1.10/" + testCase + ".html", CompositeMonitoring.buildFromList(Monitoring.make(), analysisLimiter));
