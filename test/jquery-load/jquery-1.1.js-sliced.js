 ...
 */
	 ...
	 */
        add: function(t) {
            "dk.brics.tajs.directives.unreachable";
            return this.pushStack(jQuery.merge(this.get(), typeof t == "string" ? jQuery(t).get() : t));
        },
        /**
 ...
 */
    jQuery.extend = jQuery.fn.extend = function(ARG1, ARG2, ARG3) {
        ...
        var prop;
        while (prop = arguments[a++]) // Extend the base object
        {
            for (var i in prop) {
                target[i] = prop[i];
            }
	 ...
	 */
        ...
        clean: function(a) {
            ...
            jQuery.each(a, function(i, arg) {
                ...
                if (typeof arg == "string") {
                    ...
                    !s.indexOf("<opt") && [ 1, "<select>", "</select>" ] || (!s.indexOf("<thead") || !s.indexOf("<tbody") || !s.indexOf("<tfoot")) && [ 1, "<table>", "</table>" ] || !s.indexOf("<tr") && [ 2, "<table><tbody>", "</tbody></table>" ] || // <thead> matched above
                    (!s.indexOf("<td") || !s.indexOf("<th")) && [ 3, "<table><tbody><tr>", "</tr></tbody></table>" ] || [ 0, "", "" ];
        ...
        attr: function(elem, name, value) {
            ...
            if (fix[name]) {
                ...
                "dk.brics.tajs.directives.unreachable";
                if (value == undefined && jQuery.browser.msie && elem.nodeName && elem.nodeName.toUpperCase() == "FORM" && (name == "action" || name == "method")) {
                    "dk.brics.tajs.directives.unreachable";
	 ...
	 */
        merge: function(first, second) {
            ...
            for (var i = 0, sl = second.length; i < sl; i++) // Check for duplicates
            {
                "dk.brics.tajs.directives.unreachable";
                if (jQuery.inArray(second[i], r) == -1) // The item is unique, add it
                {
                    "dk.brics.tajs.directives.unreachable";
                    first.push(second[i]);
                }
 ...
 */
    new function() {
        ...
        jQuery.browser = {
            ...
            mozilla: false && /mozilla/.test(b) && !/(compatible|webkit)/.test(b)
        };
 ...
 */
    ...
    jQuery.extend({
        ...
        /^\[ *(@)([a-z0-9_-]*) *([!*$^=]*) *('?"?)(.*?)\4 *\]/i, // Match: [div], [div p]
        /^(\[)\s*(.*?(\[.*?\])?[^[]*?)\s*\]/, // Match: :contains('foo')
        /^(:)([a-z0-9_-]*)\("?'?(.*?(\(.*?\))?[^(]*?)"?'?\)/i, // Match: :even, :last-chlid
        /^([:.#]*)([a-z0-9_*-]*)/i ],
        token: [ /^(\/?\.\.)/, "a.parentNode", /^(>|\/)/, "jQuery.sibling(a.firstChild)", /^(\+)/, "jQuery.nth(a,2,'nextSibling')", /^(~)/, function(a) {
            "dk.brics.tajs.directives.unreachable";
 ...
 */
    jQuery.event = {
        ...
        add: function(element, type, handler, data) {
            ...
            window.addEventListener("on" + type, this.handle);
        ...
        fx: function(elem, options, prop) {
            ...
            z.a = function() {
                ...
                if (prop == "opacity") {
                    ...
                    "dk.brics.tajs.directives.unreachable";
                    if (parseInt(z.now)) // My hate for IE will never die
                    {
                        "dk.brics.tajs.directives.unreachable";
            ...
            z.step = function(firstNum, lastNum) {
                ...
                if (t > options.duration + z.startTime) {
                    ...
                    if (done && jQuery.isFunction(options.complete)) // Execute the complete function
                    {
                        "dk.brics.tajs.directives.unreachable";
                    ...
                    z.now = options.easing && jQuery.easing[options.easing] ? jQuery.easing[options.easing](p, n, firstNum, lastNum - firstNum, options.duration) : // else use default linear easing
                    (-Math.cos(p * Math.PI) / 2 + .5) * (lastNum - firstNum) + firstNum;
        ...
        load: function(url, params, callback, ifModified) {
            ...
            jQuery.ajax({
                ...
                complete: function(res, status) {
                    ...
                    {
                        "dk.brics.tajs.directives.unreachable";
                        self.attr("innerHTML", res.responseText).evalScripts().each(callback, [ res.responseText, status, res ]);
                    } else {
                        "dk.brics.tajs.directives.unreachable";
	 ...
	 */
        ajax: function(s) {
            ...
            if (s.data) {
                ...
                if (s.type.toLowerCase() == "get") // "?" + data or "&" + data (in case there are already params)
                {
                    "dk.brics.tajs.directives.unreachable";
