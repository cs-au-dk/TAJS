...
public class Operators {
	...
	private static Value numeric(NumericOp op, Value v1, int register1, Value v2, int register2, Solver.SolverInterface c) {
		...
		case SUB:
			...
			if (arg1.isMaybeInf() || arg2.isMaybeInf()) 
				r = r.joinNumInf();
			if ((arg1.isMaybeNumUInt() || arg1.isMaybeSingleNumUInt()) && (arg2.isMaybeNumUInt() || arg2.isMaybeSingleNumUInt()))
				r = r.joinAnyNumUInt(); // FIXME: this rule ignores overflows from UInt (use Options.isUnsound()) 
			else
				r = r.joinAnyNumUInt().joinAnyNumOther(); // TODO: ignores overflow to +/-Infinity (use Options.isUnsound())
			break;
		...
		case DIV:
			...
			if (!arg1.isNotNum() && !arg2.isNotNum()) 
				r = r.joinAnyNumUInt().joinAnyNumOther().joinNumNaN().joinNumInf(); // TODO: (use Options.isUnsound())   - can avoid NaN here sometimes! requires isZero
			break;
		case MOD:
			if (arg1.isMaybeInf() && arg2.isMaybeInf()) 
				r = r.joinNumNaN();
			...
			if (!arg1.isNotNum() && arg2.isMaybeInf()) 
				r = r.join(arg1);
			if (!arg1.isNotNum() && !arg2.isNotNum()) 
				r = r.joinAnyNumUInt().joinAnyNumOther().joinNumNaN(); // TODO: (use Options.isUnsound())   - can avoid NaN here sometimes! requires isZero (benchpress2.js)
			break;
		}
		return r;
	}
	...
	public static Value add(Value v1, int register1, Value v2, int register2, Solver.SolverInterface c) {
		Value p1 = Conversion.toPrimitive(v1, register1, Conversion.Hint.NONE, c);
		Value p2 = Conversion.toPrimitive(v2, register2, Conversion.Hint.NONE, c);
	...
	private static Value abstractRelationalComparison(Value v1, int register1, Value v2, int register2, Solver.SolverInterface c) {
		Value p1 = Conversion.toPrimitive(v1, register1, Conversion.Hint.NUM, c);
	...
	private static Value abstractEqualityComparison(Value v1, int register1, Value v2, int register2, Solver.SolverInterface c) {
		...
		if (!v1.isNotBool()) {
			...
			if (v2.isMaybeObject()) {
				Num n1 = Conversion.fromBooltoNum(v1);
				Num n2 = Conversion.toNumber(Conversion.toPrimitive(Value.makeObject(v2.getObjectLabels()), register2, Conversion.Hint.NUM, c), register2, c);
				r = abstractNumberEquality(r, n1, n2);
			}
		}
		if (!v1.isNotNum()) {
			...
			if (v2.isMaybeObject()) {
				...
				Value arg2 = Conversion.toPrimitive(Value.makeObject(v2.getObjectLabels()), register2, Conversion.Hint.NONE, c);
		...
		if (v1.isMaybeObject()) {
			...
			if (!v2.isNotBool()) {
				Num n1 = Conversion.toNumber(Conversion.toPrimitive(vv1, register1, Conversion.Hint.NUM, c), register1, c);
				Num n2 = Conversion.fromBooltoNum(v2);
				r = abstractNumberEquality(r, n1, n2);
			}
			if (!v2.isNotNum()) {
				Value arg1 = Conversion.toPrimitive(vv1, register1, Conversion.Hint.NONE, c);
