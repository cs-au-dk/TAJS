...
public class UnevalTools {
	...
	public static NormalForm rebuildNormalForm(FlowGraph fg, CallNode n, State s, Solver.SolverInterface c) {
		...
		Set<String> args = Collections.newSet();
		String nf = p_build_nf(register, n, dr, mapping, args, Collections.<Integer>newList(), s, c, false);
	 ...
	 */
	private static String p_build_nf(int var,
                                     AbstractNode node, Decorator dr, Map<String, Integer> mapping, Set<String> arguments, List<Integer> seen_blocks, State s, Solver.SolverInterface c, boolean effects_only) {
        ...
        while (idx >= 0) {
            ...
            if (n instanceof BinaryOperatorNode) {
                ...
                if (un.getArgRegister() == var)
                    return paren(un.getOperator().toString() + paren(p_build_nf(un.getArgRegister(), node, dr, mapping, arguments, seen_blocks, s, c, effects_only)));
            }
                ...
                if (cons.getResultRegister() == var) {
                    switch (cons.getType()) {
                        case BOOLEAN:
                            return "" + cons.getBoolean();
                        case NULL:
                            return "null";
                        case NUMBER:
                            return "" + cons.getNumber();
                        case STRING:
		...
		for (BasicBlock p_bb : dr.getPredecessorBlocks(bb))
            if (!seen_blocks.contains(p_bb.getIndex())) {
                sb.append(p_build_nf(var, p_bb.getLastNode(), dr, mapping, arguments, seen_blocks, s, c, effects_only));
                seen_blocks.add(p_bb.getIndex());
            }
	...
	private static String p_build_full(int var, AbstractNode start_node, Decorator dr) {
		...
		Set<BasicBlock> p_bbs = Collections.newSet();
        ...
        while (idx >= 0) {
            ...
            if (n instanceof BinaryOperatorNode) {
                ...
                if (un.getResultRegister() == var) {
                    return paren(un.getOperator().toString() + paren(p_build_full(un.getArgRegister(), n, dr)));
                }
            }
		...
		for (BasicBlock p_bb : p_bbs)
			sb.append(p_build_full(var, p_bb.getLastNode(), dr));
		return sb.toString();
	}
	...
	public static AnalyzerCallback unevalizerCallback(final FlowGraph fg, final State state, final AbstractNode evalCall, final NormalForm input) {
		return new AnalyzerCallback() {
			...
			public Set<String> getNonGlobalIdentifiers() {
				Set<String> res = Collections.newSet();
			...
			public String getFullExpression(String ph) {
				return UnevalTools.rebuildFullExpression(fg, evalCall, input.getMapping().get(ph));
			}
		};
	}
	...
	public static String gensym() {
		return "a" + Strings.randomString(7); //FIXME: technically unsound, but sufficient
	}
}
