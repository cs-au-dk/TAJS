...
public class AnalysisSpecificMessageVarianceTester extends AbstractAnalysisVariantDifferenceTester<Boolean> {
    ...
    private static final String MESSAGE = "The variable baz has values with different types";
    ...
    @Override
    protected Boolean runAnalysisA(Path file) {
        Main.reset();
    ...
    @Override
    protected Boolean runAnalysisB(Path file) {
        Main.reset();
        return run(file);
    }

    private Boolean run(Path file) {
        Main.initLogging();
